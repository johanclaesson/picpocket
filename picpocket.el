...
;; Copyright (C) 2014 Johan Claesson
;; Author: Johan Claesson <johanclaesson@bredband.net>
;; Time-stamp: <2015-01-06 15:31:00 jcl>
;; Version: 1
...
;;    thumbnails.
;; ** This nullifies the need for undo.
...
;; PROBABLY NOT
;; * If there are more than one file in :files then that should be
;;   indicated in a message.  But we do not want to force a sha
;;   computation before showing image.  Now it is the header-line that
;;   computes sha.  Could be done from there i guess.

;; ACKNOWLEDGMENTS
;;
;; I like to thank my psycho-therapist M-x doctor for her infinite
;; patience.
...
(defvar picp-index nil
  "picp-index is starting from 1 (incompatible with elt)")
(defvar picp-length nil)
(defvar picp-width nil)
(defvar picp-height nil)
(defvar picp-header-text "")
(defvar picp-list nil
  "picp-list is a double-linked list of all images in directory.
...
(defmacro picp-measure-time (&rest forms)
  "Evaluates forms and returns (rc time-string)."
  (declare (indent defun))
...
(defun picp-edit-tags ()
  ...
  (let* ((old-tags-string (mapconcat #'symbol-name (picp-tags picp-current) " "))
    ...
    (message "Tags for %s is %s." (picp-file) (if new-tag-symbols
                                                  new-tags-string
                                                "cleared"))))
...
(defun picp-edit-keystrokes ()
  ...
  (unless picp-keystroke-alist
    (user-error (concat "You need to set picp-keystroke-alist for this command to work.\n"
                        "See `Keystroke customization' in picpocket.el header comments.")))
  (find-variable picp-keystroke-alist)
...
(defun picp-delete-file ()
  (interactive)
  (let ((file (picp-file)))
    (when (picp-y-or-n-p "Delete %s? " file)
      (picp-delete)
...
(defun picp-repeat ()
  ...
  (if picp-last-action
      (progn
        ...
        (picp-update-buffer))
    (error "No repeatable action have been done.")))
...
(defun picp-revert ()
  "Returns the new picpocket buffer."
  (interactive)
...
(defun picp-no-file-error (&optional text)
  (when text
...
(defun picp-add-tag (tag-string &optional pics)
  "Add a tag to current picture or the picture given as argument."
  (let* ((pics (or pics picp-current))
         (tag (intern tag-string))
         (tags (picp-tags pics))
         (inhibit-quit t))
    (unless (memq tag tags)
      (picp-tags-set pics (cons tag tags)))))

(defun picp-add-tag-to-all (tag-string)
  "Add a tag to all pictures in current picpocket buffer."
...
(defun picp-remove-tag-from-all (tag-string)
  "Remove a tag from all pictures in current picpocket buffer."
...
(defmacro picp-with-db (sha var-list &rest body)
  "VAR-LIST contains one or more of the symbols plist, tags and files." 
  (declare (indent defun))
...
(defun picp-db-tags-set (sha file new-tags)
  "Set the tags for the current picture."
...
(defun picp-bold (format &rest args)
  (insert (propertize (apply #'format format args) 'face 'bold)))
  ...
  (let ((start (current-time)))
    (lambda (state)
      (when (time-less-p (seconds-to-time picp-idle-timer-deadline)
        ...
        ;; (ptmp "deadline..." (current-time-string))
        ;; (ptmp "  state" (picp-calculate-index state))
        (put f
             'picp-resume-timer
...
(defun picp-compute-sha (resume-function state)
  (with-current-buffer picp-buffer
    ;; (ptmp "picp-compute-sha called" (current-time-string))
    (let ((i 0))
...
(defun picp-key-vector (key)
  (if (vectorp key)
      key
    (if (stringp key)
        (apply #'vector (listify-key-sequence key))
      (vector key))))
...
(defun picp-update-keymap ()
  (cl-loop for (key action arg) in (if (symbolp picp-keystroke-alist)
                                     ...
                                     picp-keystroke-alist)
           with keys = nil
           do (unless (member key keys)
                (define-key picp-mode-map
                  (picp-key-vector key)
                  (cond ((eq action 'tag)
  ...
  (let* ((old-dir (picp-dir))
    ...
    (while (and (file-exists-p new-path)
      ...
      (cond ((equal old-path new-path)
             (error "Attempt to %s file to itself" (picp-action-string action)))
...
(defun picp-case-fold-string-lessp (a b)
